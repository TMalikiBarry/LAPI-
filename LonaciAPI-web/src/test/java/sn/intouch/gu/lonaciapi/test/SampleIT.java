/*
package sn.intouch.gu.lonaciapi.test;
...
/**
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "LonaciAPI-ear.ear");

        // Current directory is the root of the "LonaciAPI-web" project. Go up one level
        // and enter the "LonaciAPI-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../LonaciAPI-ejb/target/LonaciAPI-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "LonaciAPI-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/LonaciAPI-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "LonaciAPI-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("sn.intouch.gu.lonaciapi.test");
