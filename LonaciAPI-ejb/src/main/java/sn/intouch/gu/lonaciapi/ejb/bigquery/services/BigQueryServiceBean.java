...
public class BigQueryServiceBean implements BigQueryService{
    ...
    @Override
    public List<Map<String, String>> getSumBetweenDatesGroupedByHour(Date startDate, Date endDate, Set<String> operators, String type,
                                                                     Boolean computeVolume) {
        try {
            BigQueryConnection connection = new BigQueryConnection();
            String query = "SELECT  " +
                    "  TIMESTAMP_TRUNC(trx.date, HOUR) as hour_timestamp, " +
                    "  SUM( " +
                    "    CASE WHEN type.category = 'BONUS' " +
                    "    THEN " +
                    "      CASE WHEN type.direction = 'CREDIT' " +
            ...
            // Build GROUP BY clause with hour_timestamp
            query += " GROUP BY hour_timestamp";
            if (operators != null) {
                query += ", trx.operateur_id";
            }
            if (type != null) {
                query += ", trx.type";
            }

            // Order by hour to get results in chronological order
